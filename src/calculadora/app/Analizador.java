...
public class Analizador implements IAnalizador {
	...
	/**
	 * Valida estructura básica: número impar de tokens y patrón OPERANDO/OPERADOR
	 * alternante.
	 *
	...
	private boolean estructuraValida(String[] tokens) {
		...
		if (tokens.length % 2 == 0)
			return false; // debe ser impar
		// No validamos el contenido aquí a fondo, solo el patrón posicional
		return true;
	}

	/**
	 * Parsea un operando: - Si es "result", devuelve resultadoActual. - Si es
	 * número, intenta convertirlo (admite coma o punto).
	 *
	...
	private TipoOperador parsearOperador(String token) {
		...
		if (t.equals("+"))
			return TipoOperador.SUMA;
		if (t.equals("-"))
			return TipoOperador.RESTA;
		if (t.equals("*"))
			return TipoOperador.MULTIPLICACION;
		if (t.equals("/"))
			return TipoOperador.DIVISION;
